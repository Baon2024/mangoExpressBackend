...
fastify.post('/outbound-call', async (request, reply) => {
 ...
 if (agentID === "agent_1301k1r16hj3ew78sh3hds1s4y8x") {
  ...
  prompt = `أنت وكيل عقاري محترف من شركة Luxury Dubai، وهي شركة رائدة في مجال العقارات في دولة الإمارات. مهمتك هي جمع إجابات على الأسئلة التالية بشكل مهذب ومباشر، ثم إنهاء المكالمة بأدب بعد الحصول على كل الإجابات المطلوبة: ${questions.join("، ")}`;
  ...
  prompt = `You are a professional real estate agent from Luxury Dubai, a leading UAE-based real estate firm. Your job is to collect answers to the following questions in a polite and direct way, then proactively end the call politely once all answers are obtained: ${questions.join(", ")}`;
