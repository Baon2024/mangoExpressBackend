...
async function getSignedUrl() {//will need to pass down agentID as param here
  try {
    const response = await fetch(
      `https://api.elevenlabs.io/v1/convai/conversation/get_signed_url?agent_id=${ELEVENLABS_AGENT_ID}`,//will need to add agentID as query parameter here
      {
        method: 'GET',
        headers: {
...
fastify.post('/outbound-call', async (request, reply) => {
  ...
  //tailor to luxuryDubai agent personality
  let questions = ["What's your name?", "What is your budget", "What area of Dubai?"]
  let prompt = `You are a concise, procurement agent. When you have collected the answers to the questions you need to ask, proactively end the call in a poli
te manner. You need to get answers to the following questions: ${questions}`
  let first_message = "Hello, I'm a Dubai property agent calling because you expressed interest in new-build Dubai property"//ideally, tailor to morning/afternoon based on function to get time
  let questionNumber = "3"//ideally, based on length of questions array of items
...
fastify.all('/outbound-call-twiml', async (request, reply) => {
  ...
  //will need to have agentID extracted from request.query
...
fastify.register(async (fastifyInstance) => {
  fastifyInstance.get('/outbound-media-stream', { websocket: true }, (ws, req) => {
    ...
    const setupElevenLabs = async () => {//will need to pass agentID here
      try {
        const signedUrl = await getSignedUrl();//will need to pass agentID here, extracted as param 
